...
The skill distribution may include the optional local audit script `skills/local-project-harness/scripts/harness_checks.mjs`. This script does not run the harness. It only checks `allowed_files` versus report change scope, secret candidates, lifecycle close state, review report logical consistency, and synchronization between root `README.md` and bundled `references/harness-readme.md`. `audit-scope --workspace <path>` defaults to the current working directory, realpath-checks existing allowed or changed paths, rejects workspace escapes and symlink targets outside the workspace, and keeps non-existing newly added files on normalized relative-path comparison. `review-logic` rejects accepted reports with failed verification, app-quality, security, scope, or secret checks; rejects accepted `work_type: app_product` reports with `not_run` or `not_applicable` app-quality checks or evidence; enforces `app_quality_failed`; checks default rubric score sums when practical; and requires `simulated_same_context` limitations to be recorded.
