...
사용자 요청
  ...
  -> goal_review_worker: Goal Contract 내용 품질 검토
  ...
  -> summary_worker: 명령 / 수행 사전 작업 / 수행 내용 / 수행 결과 형식의 Summary Report 작성
  -> 메인 세션: Summary Report 소비 후 summary_worker close
  -> 메인 세션: Summary Report 기반 최종 사용자 보고
```
...
필드 완비성 확인 뒤에는 `goal_review_worker`가 Goal Contract 내용 품질을 검토한다. 이 검토는 구현 전에 이루어져야 하며, 구현 가능성, 테스트 가능성, 범위 명확성, 안전한 가정, 실제 검증 가능성, 시나리오 플로우 충분성, 차단 질문의 적절성을 확인한다. `goal_review_worker`가 `needs_rework`, `blocked`, `rejected`를 보고하면 메인 세션은 작업 분해 전에 수정 또는 사용자 입력을 처리한다.
...
검수 보고서는 논리적으로 일관되어야 한다. `accepted`는 `passed_threshold: true`, 빈 `blocking_gates`, `overall_completion_score >= score_threshold`일 때만 가능하다. `blocking_gates`가 하나라도 있으면 `passed_threshold: false`와 비수락 상태를 보고해야 한다. `passed_threshold: false`이고 상태가 `blocked`나 `rejected`가 아니면 구체적인 `rework_items`가 있어야 한다. 같은 컨텍스트 내부 pass로 검수한 경우 `independence: simulated_same_context`를 보고하고 독립 검수로 표현하지 않는다.

## 최종 Summary Worker

모든 구현, 검증, 검수, 재작업, 통합 결정, 기존 서브에이전트 close 정리가 끝난 뒤 메인 세션은 `summary_worker`에게 최종 Summary Report 작성을 위임한다. `summary_worker`는 새 작업을 수행하거나 산출물을 재평가하지 않고 완료된 오케스트레이션 기록만 정리한다. `summary_worker`도 close audit 대상이며 Summary Report가 소비된 뒤 반드시 close되어야 한다.

Summary Report는 반드시 다음 네 항목을 포함한다.

- `명령`: 사용자의 원 요청과 메인 세션이 확정한 concrete goal
...
메인 세션은 Summary Report의 형식 완비성과 이전 보고서와의 명백한 모순 여부만 확인한다. 검수 점수나 통합 결정을 `summary_worker`에게 다시 산정하게 하지 않는다.
...
스킬 배포본의 `skills/local-project-harness/scripts/harness_checks.mjs`는 하네스를 실행하지 않는 보조 감사 도구다. 필요한 경우 `allowed_files`와 변경 보고 범위, 비밀값 후보, lifecycle close 상태, review report 논리, 루트 README와 bundled reference 동기화를 로컬에서 확인한다.
...
- `goal_review_worker`가 Goal Contract 내용 품질을 `accepted`로 검토했는가?
...
- `summary_worker`의 Summary Report가 `명령`, `수행 사전 작업`, `수행 내용`, `수행 결과`를 모두 포함하는가?
- Summary Report가 최종 사용자 보고 입력으로 소비된 뒤 `summary_worker`가 close됐는가?
