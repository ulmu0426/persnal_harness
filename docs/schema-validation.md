...
- 스킬 보조 스크립트 `skills/local-project-harness/scripts/harness_checks.mjs`의 `review-logic`, `audit-scope`, `audit-close`는 JSON Schema를 대체하지 않고 스키마만으로 표현하기 어려운 교차 필드 조건을 보조로 확인한다. Use `review-logic --report <review-report.json> --contract <goal.json>` for contract-linked reviews; it also checks app-quality gate consistency, `work_type: app_product` accepted-report evidence, accepted-report hard gates, default rubric score sums, and same-context limitation disclosure. `audit-scope --workspace <path>` resolves existing paths with realpath and rejects workspace escapes. Use `runner-policy-sync` to compare root and skill-local runner policies while allowing only expected root-to-skill-local path mappings: root `schemas/<name>.schema.json` to skill-local `references/schemas/<name>.schema.json` and root helper `skills/local-project-harness/scripts/harness_checks.mjs` to installed-skill helper `scripts/harness_checks.mjs`. The repo-root policy includes root-only maintenance sync checks; the installed skill-local policy omits those helper commands and keeps only installed-safe helper audits. Any other helper command, budget, gate, or policy drift must fail.
...
- 리뷰 보고서에 `overall_completion_score`, `score_threshold`, `passed_threshold`, `blocking_gates`, `rework_items`, `app_quality_check` 누락
