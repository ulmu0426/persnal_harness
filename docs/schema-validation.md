...
- 스킬 보조 스크립트 `skills/local-project-harness/scripts/harness_checks.mjs`의 `review-logic`, `audit-scope`, `audit-close`는 JSON Schema를 대체하지 않고 스키마만으로 표현하기 어려운 교차 필드 조건을 보조로 확인한다. `review-logic` also checks app-quality gate consistency, `work_type: app_product` accepted-report evidence, accepted-report hard gates, default rubric score sums, and same-context limitation disclosure. `audit-scope --workspace <path>` resolves existing paths with realpath and rejects workspace escapes.
