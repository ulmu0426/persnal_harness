...
- 모든 작업, 검수, 재작업, 통합 결정, 기존 서브에이전트 close 정리가 끝나면 메인 세션은 최종 사용자 보고 직전에 `summary_worker`에게 전체 수행 내용을 총정리하도록 위임한다.
- `summary_worker`는 새 작업을 수행하거나 산출물을 평가하지 않고, 이미 완료된 오케스트레이션 기록만 바탕으로 `명령`, `수행 사전 작업`, `수행 내용`, `수행 결과` 형식의 Summary Report를 작성한다.
...
11. 모든 작업 종료 후 메인 세션은 `summary_worker`에게 최종 정리를 위임하고, Summary Report가 소비되면 `summary_worker`를 close한다.
12. 실제 러너 구현 전까지 `docs/runner-cli.md`의 명령은 dry-run, record, audit 계약으로만 취급한다.
...
- 최종 정리 위임: 모든 작업과 기존 서브에이전트 close 정리가 끝난 뒤 `summary_worker`에게 Summary Report 작성을 위임한다.
- 최종 사용자 보고: `summary_worker`의 Summary Report를 기반으로 사용자에게 최종 보고한다.
...
`summary_worker`는 모든 구현, 검증, 검수, 재작업, 통합 결정, 기존 서브에이전트 close 정리가 끝난 뒤에만 실행된다. `summary_worker`는 새 파일을 수정하거나 산출물을 재평가하지 않고, 완료된 Goal Contract, 위임 기록, 워커 보고서, 검수 보고서, 검증 결과, rework 이력, hard gate 상태, close 상태를 읽어 최종 Summary Report만 작성한다. Summary Report는 반드시 다음 네 항목을 포함한다.

- `명령`: 사용자의 원 요청과 메인 세션이 확정한 concrete goal
- `수행 사전 작업`: Goal Contract 작성, 범위 설정, 위임 계획, 검증 계획, 사전 조사처럼 실제 변경 전에 수행된 준비 작업
- `수행 내용`: 어떤 하위 에이전트가 어떤 작업을 맡았고, 어떤 구현, 검증, 검수, 재작업, 합의, close 처리가 수행됐는지
- `수행 결과`: 최종 통합 결정, 검증 결과, 완료 여부, 미검증 항목, 남은 위험, 후속 권고

메인 세션은 Summary Report의 형식 완비성만 확인하고, 이미 완료된 검수 점수나 통합 결정을 `summary_worker`에게 다시 산정하게 하지 않는다.
`summary_worker`도 close audit 대상이며, Summary Report가 최종 사용자 보고 입력으로 소비된 뒤 반드시 close되어야 한다.

## 스키마 검증과 러너 정책

v1 하네스는 실제 실행 러너를 구현하지 않는다. 대신 `schemas/`와 `harness/runner_policy.yaml`로 Goal Contract, 위임 계약, 워커 보고서, 리뷰 보고서, Summary Report, 수명주기 이벤트, 실행 상태의 구조를 고정한다.
...
사용자 요청
  ...
  -> summary_worker: 명령 / 수행 사전 작업 / 수행 내용 / 수행 결과 형식으로 총정리
