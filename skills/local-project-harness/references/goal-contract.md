...
- For vague game requests, infer a `game_brief` instead of asking broad preference questions by default:
  ...
  - `assets`: local-only sprites, models, textures, audio placeholders, UI art, and provenance expectations. If generated raster game assets are needed, route them to `game_asset_worker` and require active `$imagegen` built-in usage, workspace-saved outputs, final prompt/path reporting, a provenance/integration manifest, and one generation call per distinct asset or variant.
...
- For generated raster game assets, seed `game_asset_worker` instead of generic `asset_worker`. The delegation must prohibit external asset stores, credential assumptions, deployment, unauthorized network or external cost, and paid generation paths unless separately approved; it must require `$imagegen` built-in mode by default, chroma-key plus `remove_chroma_key.py` first for transparent/cutout assets, explicit user confirmation before CLI fallback, workspace persistence of project-bound outputs, final prompt and saved-path reporting, and a provenance/integration manifest.
